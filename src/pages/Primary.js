...
function Primary() {
  ...
  if (selectedImage) {
    const reader = new FileReader();
    const fileType = selectedImage.type.split('/')[1].toUpperCase(); // 'image/png' -> 'PNG'
  ...
  if (selectedImage) {
    const reader = new FileReader();
    // Détermine le type de l'image (par exemple, 'image/jpeg' -> 'JPEG')
    const fileType = selectedImage.type.split('/')[1].toUpperCase();

    reader.onload = (e) => {
      // Stocke l'URL des données et le type de fichier
      setBackgroundImage({
        dataUrl: e.target.result,
        fileType: fileType
      });
    ...
    try {
      ...
      for (const eleve of data) {
        for (const matiere of allMatiere) {
          ...
          if (backgroundImage) {
            doc.addImage(backgroundImage.dataUrl, backgroundImage.fileType, currentX, currentY, stickerWidth, stickerHeight);
          }
          ...
          if (logoEcole) {
            doc.addImage(logoEcole.dataUrl, logoEcole.fileType, currentX + 0.3, currentY + 0.3, 1.2, 1.2);
          }

          if (logoCI) {
            doc.addImage(logoCI.dataUrl, logoCI.fileType, currentX + stickerWidth - 1.5, currentY + 0.3, 1.2, 1.2);
          }
          ...
          doc.text(`${(eleve.classe || '').toUpperCase()}`, currentX + 0.5, currentY + 2.9);

          doc.setFontSize(6)
          doc.setTextColor(239, 35, 60);
          doc.text(`MATRICULE: ${(eleve.matricule || '').toUpperCase()}`, currentX + 0.5, currentY + 3.4);
